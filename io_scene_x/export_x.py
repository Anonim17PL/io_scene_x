...
class MeshExportObject(ExportObject):
    ...
    def Write(self):
        ...
        if self.Config.ExportMeshes:
            ...
            if self.Config.ApplyModifiers:
                ...
                Mesh = self.BlenderObject.to_mesh(preserve_all_data_layers=False, depsgraph=depsgraph)
    ...
    def __WriteMeshMaterials(self, Mesh):
        print("__WriteMeshMaterials")
        def WriteMaterial(Exporter, Material):
            print("WriteMaterial")
            def GetMaterialTextureFileName(Material):   
                #for x in mat_slot.material.node_tree.nodes:
                if Material.node_tree.nodes["Principled BSDF"].inputs[0].links:
                    x = Material.node_tree.nodes["Principled BSDF"].inputs[0].links[0].from_node
                    if x.type=='TEX_IMAGE':
                        return x.image.name
                return None
            ...
            print(Material.node_tree.nodes["Principled BSDF"].inputs[0].default_value[0])
            print(Material.node_tree.nodes["Principled BSDF"].inputs[5].default_value)
            DiffuseR = (Material.node_tree.nodes["Principled BSDF"].inputs[0].default_value[0])
            DiffuseG = (Material.node_tree.nodes["Principled BSDF"].inputs[0].default_value[1])
            DiffuseB = (Material.node_tree.nodes["Principled BSDF"].inputs[0].default_value[2])
            DiffuseA = (Material.node_tree.nodes["Principled BSDF"].inputs[0].default_value[3])
            
            # Map Blender's range of 1 - 511 to 0 - 1000
            #Specularity = 1000 * (Material.specular_hardness - 1.0) / 510.0
            Specularity = 40
            
            Specular = (Material.node_tree.nodes["Principled BSDF"].inputs[5].default_value)
            
            EmissionR = (Material.node_tree.nodes["Principled BSDF"].inputs[17].default_value[0])
            EmissionG = (Material.node_tree.nodes["Principled BSDF"].inputs[17].default_value[1])
            EmissionB = (Material.node_tree.nodes["Principled BSDF"].inputs[17].default_value[2])
            
            Exporter.File.Write("{:9f};{:9f};{:9f};{:9f};;\n".format(DiffuseR,
                DiffuseG, DiffuseB, DiffuseA))
    ...
    def __WriteMeshActiveImageMaterials(self, Mesh):
        ...
        for Material in enumerate(mesh_materials):
            WriteMaterial(self.Exporter, Material)
    ...
    def __WriteMeshSkinWeights(self, Mesh, MeshEnumerator=None):
        ...
        for ArmatureObject in ArmatureObjects:
            ...
            for Index, Vertex in enumerate(MeshEnumerator.vertices):
                ...
                # to armature bones.
                for VertexGroup in Vertex.collections:
...
class GenericAnimationGenerator(AnimationGenerator):
    ...
    def _GenerateKeys(self):
        ...
        Scene.subframe=(BlenderCurrentFrame)
...
classes = (
    ...
    Animation,
    AnimationGenerator,
    GenericAnimationGenerator,
    GroupAnimationGenerator,
    ArmatureAnimationGenerator,
    AnimationSet,
    AnimationWriter,
    JoinedSetAnimationWriter,
    SplitSetAnimationWriter,
    File,
    Util,
    )
